...
if Config.SharedEmotesEnabled then
    ...
    RegisterCommand('neardance', function(source, args, raw)
        if #args > 0 then
            local emotename = string.lower(args[1])
            target, distance = GetClosestPlayer()
            if(distance ~= -1 and distance < 3) then
                if DP.Dances[emotename] ~= nil then
                    _,_,rename = table.unpack(RP.Dances[emotename])
                    TriggerServerEvent("ServerEmoteRequest", GetPlayerServerId(target), emotename, 'Dances')
                    SimpleNotify(Config.Languages[lang]['sentrequestto']..GetPlayerName(target).." ~w~(~g~"..rename.."~w~)")
                else
                    EmoteChatMessage("'"..emotename.."' "..Config.Languages[lang]['notvalidsharedemote'].."")
                end
            else
...
AddEventHandler("SyncPlayEmoteSource", function(emote, player)
    ...
    if (RP.Shared[emote]) and (RP.Shared[emote].AnimationOptions) then
      local SyncOffsetFront = RP.Shared[emote].AnimationOptions.SyncOffsetFront
